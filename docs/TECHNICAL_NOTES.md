...
| Projectile silently double-drawn (once via the wrong pipeline layout) whenever Mixed Materials + Grid Alpha < 1 + an active projectile all held at once, introduced by the roadmap.md "Refactor backlog" #2 dedup of the M3 interleave logic | The extracted `drawBucketWithInterleavedProjectile` gated interleaving on `projectileNeedsInterleave` alone - a single outer, bucket-independent flag - so *both* the unconditional normal-bucket call and the `if(mixed)`-guarded special-bucket call independently decided to interleave whenever `mixed && transparent`, each unaware the other bucket was also about to draw the projectile. The normal-bucket copy was the more serious half: it ran with `pipeline_` actually bound, while `drawProjectile()` pushes descriptors/constants against the outer `activePipeline`'s (`transparentPipeline_`'s) layout - a real pipeline-layout mismatch, not just a harmless extra draw | Never caught by testing because it's visually near-silent: redrawing the same projectile at the same transform mostly fails the depth test against the first copy, so nothing looks wrong on screen even though the command buffer is doing (and risking) something incorrect. Found by re-deriving the interleave logic from scratch while writing up the "disputed items" analysis, not by running the app. The general lesson: a helper extracted from code that was only ever called from mutually-exclusive branches (`!mixed`-gated vs. `mixed`-gated, here) can silently stop being mutually exclusive once the branches are unified - the exclusivity has to be re-derived explicitly (`isTransparentBucket`, §47's follow-up fix), not assumed to survive the refactor. Fixed by ANDing the bucket's own "is this bucket alpha-blended" value into the interleave gate, which was already being computed at each call site for an unrelated reason (the non-interleaved `reverseOrder` value) and just needed reusing rather than a new parameter |

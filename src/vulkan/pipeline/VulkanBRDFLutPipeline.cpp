...
void VulkanBRDFLutPipeline::create(
    VkDevice device,
    VkExtent2D extent,
    VkRenderPass renderPass)
{
    ...
    VkShaderModule fragModule =
        ShaderLoader::load(device, "shaders/compiled/brdfLUT.frag.spv");
    ...
    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &info, nullptr, &pipeline_) != VK_SUCCESS)
        throw std::runtime_error("Failed to create BRDF LUT pipeline");

    vkDestroyShaderModule(device, vertModule, nullptr);
    vkDestroyShaderModule(device, fragModule, nullptr);
}

void VulkanBRDFLutPipeline::destroy(VkDevice device)
{
